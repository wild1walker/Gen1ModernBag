...
-- The DEFAULT is OFF: a press is a row, and nothing repeats on its own.  Any
-- hold-to-scroll setting has a threshold, and the threshold is what made this
-- feel wrong -- the same press is one step or a run of them depending on how
-- long a finger rests.  Both the schema's default and the code's own fallback
-- are asserted, because a mod whose two defaults disagree has a default
-- nobody can name.
assert(optionSchema[2].default == "off", "Hold Scroll Speed default is not OFF")
optionValues.hold_scroll_speed = nil
local unset = BagMenu.new(game, {})
assert(unset.keyRepeat == false,
  "an unset Hold Scroll Speed did not fall back to OFF")
